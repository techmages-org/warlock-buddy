...
// ──────────────── shared geometry ────────────────
// Tuned for CoreS3: pet lives in the left 160px column (x=0..160) of the
// 320×240 landscape, vertically centered in the 176px body area between
// status bar (y=0..24) and touch bar (y=200..240).
const int BUDDY_X_CENTER = 80;     // center of left half
// Clear stops at x=158 — the bubble outline lives at x=160 and the pet's
// per-tick fillRect was erasing its left edge.
const int BUDDY_CANVAS_W = 158;
const int BUDDY_Y_BASE   = 47;     // scale-2 body lands at y≈80..152
const int BUDDY_Y_OVERLAY = 20;    // particles above body, below status bar
...
// M5.Display (both M5Canvas and M5GFX inherit LovyanGFX). Coords stay
...
// One-shot render to an arbitrary M5GFX surface (M5.Display direct, not the
// canvas). Bypasses tick gating and the sprite fillRect — caller owns
...
void buddyTick(uint8_t personaState) {
  ...
  // Clear the pet column — start at y=24 to leave the status bar intact
  // (CoreS3-specific; status bar is owned by main.cpp).
  const int CLEAR_TOP = 24;
  spr.fillRect(0, CLEAR_TOP, BUDDY_CANVAS_W,
               (BUDDY_Y_BASE + 5 * BUDDY_CHAR_H + 12) * _scale - CLEAR_TOP, BUDDY_BG);
