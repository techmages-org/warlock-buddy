...
void bleInit(const char* deviceName) {
  ...
  // Just Works LESC pairing (no MITM passkey ceremony). The combination of
  // ENCRYPT_MITM + ESP_LE_AUTH_REQ_SC_MITM_BOND + ESP_IO_CAP_OUT triggers a
  // long-standing Bluedroid bug where SecCallbacks don't fire when paired
  // with non-Apple-native centrals (e.g. Bun/@abandonware/noble on macOS):
  //   https://github.com/espressif/esp-idf/issues/2036
  //   https://github.com/espressif/arduino-esp32/issues/7251
  // ECDH key exchange under LESC still protects against passive sniffing;
  // we lose only MITM authentication. TODO: restore passkey flow once we
  // figure out the right Bluedroid configuration for the S3 + noble combo.
  ...
  // KNOWN REGRESSION (cores3 v0.1.0): characteristics are NOT encrypted.
  // The ENCRYPTED variants triggered the SecCallbacks-don't-fire bug on
  // S3+Bluedroid+macOS-CoreBluetooth; pairing would silently time out.
  // Anyone in BLE range can read transcript snippets and tool-call hints
  // until we fix the pairing flow. TODO: revisit (see ble_bridge.cpp comment
  // in bleInit() for references).
  ...
  sec->setAuthenticationMode(ESP_LE_AUTH_REQ_SC_BOND);   // LESC + bond, no MITM
  sec->setCapability(ESP_IO_CAP_NONE);                    // NoInputNoOutput → Just Works
  sec->setKeySize(16);
