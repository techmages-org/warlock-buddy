...
// CoreS3 layout: pet column is the left half (x=0..160), body area runs
// y=24..200 (24px status bar above, 40px touch bar below). Center the GIF
// in that column. Peek mode (used by future PET/INFO sub-screens) renders
// at half scale.
static const int PET_COL_W = 160;
static const int BODY_Y_TOP = 24;
static const int BODY_HEIGHT = 176;
...
void characterSetState(uint8_t s) {
  ...
  if (gif.open(full, gifOpenCb, gifCloseCb, gifReadCb, gifSeekCb, gifDrawCb)) {
    ...
    gifPlace();
    // Clear only the pet column — the bubble + status bar are owned by
    // main.cpp and a full fillSprite would wipe them.
    spr.fillRect(0, BODY_Y_TOP, PET_COL_W, BODY_HEIGHT, pal.bg);
    nextFrameAt = 0;
...
void characterTick() {
  ...
  if (!gif.playFrame(false, &delayMs)) {
    // End of animation. Single-gif states LOOP via gif.reset() — that
    // rewinds within the open file handle, no LittleFS reopen, no GIF
    // header re-decode. The StickC used close+freeze here to avoid
    // starving the BT controller; the CoreS3's ESP32-S3 has the headroom
    // to loop continuously and the operator gets actual animation.
