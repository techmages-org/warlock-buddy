...
// 5M output tokens per level — calibrated against real
// `/api/state.cost.this_month[].total_tokens.output` cumulative data at
// Jason's multi-account cadence. Demos at 100K threshold confirmed the
// mechanism end-to-end (2026-05-24); 5M gives a satisfying multi-hour
// level-up cadence during heavy use, no level-up on quiet days.
// 10 pips per level → 500K tokens per pip — visibly progresses within a
// single working session.
static const uint32_t TOKENS_PER_LEVEL = 5000000;
